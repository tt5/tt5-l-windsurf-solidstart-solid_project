...
let g$1 = class g {
  ...
  async add(t, s, e) {
    ...
    try {
      ...
      try {
        ...
        const u = await this.db.get("SELECT id, user_id as userId, x, y, game_created_at_ms as createdAtMs FROM base_points WHERE id = ?", [d.lastID]);
        if (!u) throw new Error("Failed to retrieve created base point");
        return u;
      } catch (a) {
  ...
  async deletePoints(t) {
    ...
    if (s) {
      ...
      try {
        ...
        const { basePointEventService: e } = await import('./base-point-events-C2UNy6C4.mjs'), r = { ...s, count: t.length };
  ...
  async deleteBasePoint(t) {
    ...
    try {
      ...
      const { basePointEventService: e } = await import('./base-point-events-C2UNy6C4.mjs');
...
class O {
  ...
  async saveTile(t) {
    ...
    if (!r && t.data) {
      ...
      a[0] = 1;
      const i = deflate(t.data, { level: M });
...
__publicField(O, "TILE_SIZE", 64);
const A = process.env.DATA_DIR || join(process.cwd(), "data"), _ = join(A, "app.db");
let o, l = null, E = null;
async function b() {
  if (!o) {
    console.log("Initializing database connection...");
    try {
      await promises.mkdir(dirname(_), { recursive: true }), o = await open({ filename: _, driver: c.Database, mode: c.OPEN_READWRITE | c.OPEN_CREATE }), await o.exec("PRAGMA journal_mode = WAL;"), await o.exec("PRAGMA foreign_keys = ON;"), console.log("Database connection established");
    } catch (n) {
...
export { D, O, b, g$1 as g, x };
//# sourceMappingURL=db-97-DOlOW.mjs.map
